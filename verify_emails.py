...
class EmailVerifier:
    ...
    def verify_email(self, email: str) -> Tuple[str, str]:
        ...
        Returns:
            Tuple of (status, details) where status is 'Valid' or 'Invalid'
        """
        email = email.strip()
            ...
            try:
                ...
                with smtplib.SMTP(mx_host, port=25, timeout=10) as server:
                    ...
                    if code == 250: # Success
                        ...
                        return ('CATCH-ALL', f'L3: Ambiguous acceptance by {mx_host} (Code {code})')
                    elif code >= 500: # 5xx codes are permanent failures (rejection/block)
